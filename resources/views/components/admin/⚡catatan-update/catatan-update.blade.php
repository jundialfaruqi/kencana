<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">Ubah Catatan</h1>
            <p class="text-sm text-base-content/60 mt-1">Perbarui catatan aturan</p>
        </div>
        <div>
            ...
            <div class="card-body">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="card md:col-span-2 p-4 bg-base-200 text-base-content mt-1">
                            ...
                            <div class="flex flex-wrap gap-2 mt-1.5" wire:ignore.self>
                                @foreach ($availableKategoriCatatan as $kategori)
                                    <button type="button" wire:key="{{ $kategori }}"
                                        class="btn btn-sm {{ $selectedKategoriCatatan === $kategori ? 'btn-secondary' : 'btn-outline' }}"
                        ...
                        <label class="form-control w-full md:col-span-2">
                            <div class="label">
                                <span class="label-text font-medium tex-base-content">Atau Ketik Kategori Catatan
                                    Baru</span>
                            </div>
                            <label class="input input-bordered flex items-center w-full gap-2 mt-1.5">
                                <input type="text"
                                    ...
                                    x-data="{}" x-bind:disabled="$wire.selectedKategoriCatatan !== null">
                ...
                <div class="mt-6 flex items-center justify-end gap-2">
                    <a wire:navigate href="{{ route('catatan') }}" class="btn btn-ghost" wire:loading.attr="disabled"
                        wire:target="submit">
                        Kembali
                    </a>
                    <button type="button" class="btn btn-accent" wire:click="submit" wire:loading.attr="disabled"
                        wire:target="submit">
                        <span wire:loading.remove wire:target="submit">Perbarui Catatan</span>
