<div id="lapangan-create-root">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">Buat Lapangan</h1>
            <p class="text-sm text-base-content/60 mt-1">Tambah data lapangan baru</p>
        </div>
        <div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="form-control w-full">
                        ...
                        <div class="mt-2 relative group aspect-video w-full rounded-2xl border-2 border-dashed border-base-300 bg-base-200 hover:border-accent overflow-hidden flex flex-col items-center justify-center cursor-pointer transition-all"
                            ...
                            <!-- Empty State -->
                            <div class="flex flex-col items-center justify-center text-center p-4" id="cover-empty"
                                style="{{ $image_cover ? 'display: none;' : '' }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            ...
                            <!-- Close Button -->
                            <button type="button"
                                class="btn btn-circle btn-xs btn-error text-white absolute top-2 right-2 z-10 {{ $image_cover ? '' : 'hidden' }}"
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full">
                            @foreach ($images as $idx => $file)
                                <div class="relative group aspect-square rounded-2xl border-2 border-dashed border-base-300 bg-base-200 hover:border-secondary overflow-hidden flex flex-col items-center justify-center cursor-pointer transition-all"
                                    ...
                                    <!-- Close Button -->
                                    <button type="button"
                                        class="btn btn-circle btn-xs btn-error text-white absolute top-2 right-2 z-10 {{ $file ? '' : 'hidden' }}"
    ...
    <script>
        (function() {
            function compressAndUpload(file, fieldName, statusEl, progressBar, onComplete, onError) {
                ...
                reader.onload = function(event) {
                    ...
                    img.onload = function() {
                        ...
                        // Scale down image dimensions if too large
                        const maxDim = 1000;
                        ...
                        function attemptCompress() {
                            canvas.toBlob(function(blob) {
                                ...
                                if (blob.size <= maxSizeBytes || quality <= minQuality) {
                                    ...
                                    // Upload using Livewire Javascript API
                                    const root = document.getElementById('lapangan-create-root');
                                    ...
                                    // Iteratively lower quality until size fits
                                    quality -= 0.05;
                                    attemptCompress();
                                }
                            }, 'image/webp', quality);
            ...
            window.removeCoverImage = function() {
                ...
                window.Livewire.find(componentId).set('image_cover', null);
            };
            ...
            window.removeGalleryImage = function(index) {
                ...
                const root = document.getElementById('lapangan-create-root');
                const componentId = root.closest('[wire\\:id]').getAttribute('wire:id');
                window.Livewire.find(componentId).call('removeImage', index);
            };

            document.addEventListener('DOMContentLoaded', function() {
                const root = document.getElementById('lapangan-create-root');
                if (root) {
                    root.addEventListener('form-reset', function() {
                        removeCoverImage();
                        for (let i = 0; i < 4; i++) {
