<div class="flex-1 flex flex-col min-h-0 overflow-hidden bg-white" wire:init="loadData">
    <div class="flex-1 overflow-y-auto min-h-0">
        ...
        <div class="px-4 py-4 space-y-5">
            ...
            @if ($isLoading)
                {{-- Banner Skeleton --}}
                <div class="relative w-full aspect-video rounded-2xl sm:rounded-3xl bg-gray-200 animate-pulse overflow-hidden shadow-md shadow-gray-200/60 p-4 sm:p-7 flex flex-col justify-end gap-2">
                    <div class="h-5 sm:h-7 bg-gray-300 rounded-lg w-2/3"></div>
                    <div class="h-3 sm:h-4 bg-gray-300/80 rounded w-1/2"></div>
                    <div class="flex justify-center items-center gap-1.5 mt-2">
                        <div class="w-6 h-1.5 sm:h-2 bg-gray-300 rounded-full"></div>
                        <div class="w-2 h-1.5 sm:h-2 bg-gray-300 rounded-full"></div>
                        <div class="w-2 h-1.5 sm:h-2 bg-gray-300 rounded-full"></div>
                    </div>
                </div>
            @elseif (!empty($banners))
                <div x-data="webviewBannerSlider({{ count($banners) }})" @mouseenter="stopAutoPlay()" @mouseleave="startAutoPlay()"
                    class="relative w-full">

                    {{-- Carousel Track --}}
                    <div x-ref="slider" @scroll.debounce.50ms="onScroll($event)"
                        class="flex overflow-x-auto snap-x snap-mandatory scroll-smooth rounded-2xl sm:rounded-3xl shadow-md shadow-gray-200/60 aspect-video m:aspect-21/9 w-full"
            ...
            <div>
                ...
                @if ($isLoading)
                    {{-- Lapangan Skeleton Loader --}}
                    <div class="flex gap-3.5 overflow-x-auto pb-2 pt-1" style="scrollbar-width: none;">
                        @for ($i = 0; $i < 2; $i++)
                            <div
                                class="w-[88%] sm:w-95 shrink-0 rounded-3xl bg-white border border-gray-100 shadow-lg shadow-gray-200/50 overflow-hidden flex flex-col animate-pulse">
                                <div class="w-full aspect-16/10 bg-gray-200"></div>
                                <div class="p-4 flex-1 flex flex-col justify-between space-y-4">
                                    <div class="space-y-2">
                                        <div class="h-3 bg-gray-200 rounded-full w-14"></div>
                                        <div class="h-4.5 bg-gray-200 rounded-md w-3/4"></div>
                                        <div class="h-3 bg-gray-100 rounded w-1/2"></div>
                                    </div>
                                    <div class="pt-3 border-t border-gray-100">
                                        <div class="h-10 bg-gray-200 rounded-2xl w-full"></div>
                                    </div>
                                </div>
                            </div>
                        @endfor
                ...
                @elseif (empty($arenas))
                    <div class="p-6 rounded-2xl bg-gray-50 border border-gray-100 text-center">
                        <div class="text-xs font-black text-gray-700">Tidak Ada Lapangan Tersedia</div>
                        <div class="text-[11px] text-gray-400 mt-0.5">Silakan periksa kembali nanti</div>
                    </div>
                @else
                                        ...
                                        <div class="mt-4 pt-3 border-t border-gray-100">
                                            @if ($isOpen)
                                                <a href="{{ route('webview.pesan', ['lapangan_id' => $arena['id']]) }}"
                                                    wire:navigate x-data="{ loading: false }" @click="loading = true"
            ...
            <div>
                ...
                @if ($isLoading)
                    {{-- Histori Booking Skeleton Loader --}}
                    <div class="space-y-3">
                        @for ($i = 0; $i < 3; $i++)
                            <div class="p-3.5 rounded-2xl bg-white border border-gray-100 shadow-sm animate-pulse space-y-2.5">
                                <div class="flex items-center justify-between">
                                    <div class="h-3 bg-gray-200 rounded w-20"></div>
                                    <div class="h-4 bg-gray-200 rounded-full w-14"></div>
                                </div>
                                <div class="h-4 bg-gray-200 rounded w-1/2"></div>
                                <div class="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100/80">
                                    <div class="h-3 bg-gray-100 rounded w-24"></div>
                                    <div class="h-3 bg-gray-100 rounded w-20 justify-self-end"></div>
                                </div>
                            </div>
                        @endfor
                    </div>
                @elseif (empty($recentBookings))
                    <div class="p-6 rounded-2xl bg-gray-50 border border-gray-100 text-center">
                        <div
                        ...
                        <div class="text-[11px] text-gray-400 mt-0.5">Pemesanan yang Anda buat akan muncul di sini
                        </div>
                    </div>
                @else
                    <div class="space-y-3">
                        @foreach ($recentBookings as $booking)
                            @php
                                ...
                                if (preg_match('/^\d{4}-\d{2}-\d{2}/', $tglRaw)) {
                                    try {
                                        $tglFmt = \Carbon\Carbon::parse($tglRaw)
                                            ->locale('id')
                                ...
                                if (
                                    empty($jamPlay) &&
                                    (!empty($booking['jam_mulai']) || !empty($booking['jam_selesai']))
                                ) {
