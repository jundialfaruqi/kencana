    ...
    <script>
        (function() {
            function showToast(detail) {
                ...
                if (!container) return;
                var payload = detail && detail.detail ? detail.detail : detail;
                if (Array.isArray(payload)) payload = payload[0] || {};
                var type = (payload && payload.type) || 'success';
                var title = (payload && payload.title) || (type === 'error' ? 'Gagal' : (type === 'info' ? 'Info' :
                    'Berhasil'));
                ...
                alert.className = 'alert bg-black text-white shadow-xl rounded-3xl border-0';
            ...
            function bindToast() {
                if (window.__publicToastBound) return;
                document.addEventListener('toast', function(e) {
                    showToast(e.detail || {});
                }, {
                    passive: true
                });
                if (window.Livewire && window.Livewire.on && !window.__publicToastLWBound) {
